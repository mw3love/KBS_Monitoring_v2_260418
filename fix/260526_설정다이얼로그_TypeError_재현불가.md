...
(4차 조사 중 dev의 `kbs_config.json`에서 `true`(13:46)로 켜져 있는 것을 발견 — 운영 PC 배포 전 반드시 확인할 것.)

- **~7일 넘게 무사** → arm 3·4·5 중 하나가 범인. 셋 다 없어도 되는 것들이므로 **그대로 두고 종결**.
- **또 ~5일에 죽음** → **셋 다 무죄**. 남는 것은 PySide6 본체 / Python 3.14 → **Round 2: Python 3.13 다운그레이드**(코드 무변경).
- 어느 쪽이든 **onset 텔레그램 덕분에 10분 안에 알게 된다.**

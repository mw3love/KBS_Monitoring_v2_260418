...
> **최신은 §13-8 (5차 + Round 1 부결 + Round 2 운영 PC 실배포 완료·판정 시계 가동 중, ~07-29~30 재발 여부 관찰)**. 여기부터 읽는 게 빠르다.
...
- **텔레그램 기동 알림 최초 무응답 → 원인 규명 → 해결**: 처음엔 텔레그램이 안 옴 → 조사 결과 사용자가 **바탕화면에 저장해 둔 예전 백업 설정 파일을 계속 재사용**하고 있었음(신버전에 새로 생긴 설정 키가 옛 백업엔 없었을 가능성). 설정을 다시 저장→불러오기(재직렬화로 `_merge_defaults` 갱신 추정) 후 재기동 → **텔레그램 정상 수신 확인.**

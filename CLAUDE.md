...
- ⚠ **이 PC는 다른 프로젝트 작업(Claude Code 등)과 python.exe 프로세스 공간을 공유한다.** 다른 세션이
  자기 프로젝트 정리용으로 `taskkill /IM python.exe`처럼 **이름 기준(PID 미지정)** 명령을 쓰면 KBS의
  watchdog·detection·main이 전부 같이 죽는다. 실제로 2026-08-31 11:46경 발생 — watchdog·detection·main
  세 프로세스 PID가 크래시 흔적(fault.log·ui_degraded.flag·Windows 이벤트 로그) 전혀 없이 동시에 바뀌었고,
  `last_exit.json`도 안 갱신돼(정상 종료 경로 미실행) 외부 강제종료로 확인됐다.
  → **다행히 `실행.bat`의 재시작 루프가 자동으로 되살렸다**(프로세스 트리상 8/12부터 떠 있던 `실행.bat`의
  cmd.exe가 새 `main.py`를 직접 자식으로 재기동). 단 이 루프는 원래 예약재시작 종료코드(42) 전용으로 설계돼
  있는데 강제종료로도 돌았다.
  → **"42로 우연히 떨어졌다"는 아니다** — PID 하나만 정확히 지정한 `taskkill /F /PID`로 격리 재현했더니
  `%errorlevel%`은 1이었다(scratchpad에서 검증, KBS·다른 세션 프로세스 영향 없음). 즉 그 루프는 원래
  설계대로면 오늘 안 돌았어야 정상.
  → **유력 가설(정황증거, 미확정)**: `실행.bat`를 돌리는 그 cmd.exe(PID 17544)는 **2026-08-12 09:49:24**에
  떴는데, 같은 날 **10:02**에 커밋 `0d53c0f`가 바로 이 파일의 `:loop` 이전 구간(venv 폴백 블록)을 수정했다
  — 즉 그 cmd.exe가 파일을 읽기 시작한 지 13분 뒤에 파일이 바뀌었다. batch의 `goto`는 파일을 매번 처음부터
  다시 읽지 않고 **바이트 위치(책갈피) 기준**으로 재탐색하므로, 루프 도중 파일이 편집되면 그 프로세스는
  이후 계속 어긋난 위치를 읽을 수 있다(널리 알려진 배치 함정: 실행 중인 .bat 수정 금지). 이 cmd.exe는
  8/12 이후 한 번도 재시작 안 된 채 19일째 그 어긋난 상태로 돌고 있었을 가능성이 있다 — 오늘의 "예약
  신호 없이도 재시작" 이상동작과 시점이 들어맞는다. 다만 실제 재현은 라이브 프로세스라 위험해 보류,
  **증명된 인과관계 아님**.
  → **대응**: `실행.bat`을 편집하는 커밋을 배포한 뒤에는, `main.py`만 재시작하지 말고 **그 배치를 돌리는
  cmd.exe 자체를 통째로 껐다 새로 켤 것**(현재 그대로 두면 다음 배포 후에도 같은 어긋남이 누적될 수 있음).
  크래시류 원인(6차 힙손상 등)부터 의심하지 말고 **이 패턴(크래시 흔적 없는 전체 동시 재기동)이면 먼저
  이 항목부터 확인**할 것.

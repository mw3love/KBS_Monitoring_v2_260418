...
| `SignoffStateChange` | `group_id:int(1\|2)`, `prev_state:str`, `new_state:str('IDLE'\|'PREPARATION'\|'SIGNOFF')`, `source:str('auto-time'\|'auto-detect'\|'auto-revert'\|'manual'\|'restore')`, `suppress_alarm_sound:bool`, `message:str` | 정파 상태 전환. `auto-revert`=해제준비 시각 전 오감지 SIGNOFF의 PREPARATION 조기복귀. `suppress_alarm_sound`=플래핑 묶음 모드 중 전환 → UI가 정파 알림음 억제(docs_signoff_운영.md §8). `message`=사람이 읽는 전환 문장(그룹명 포함, signoff_manager가 구성) — UI 로그 위젯이 이 문장에 소스/경과시간을 붙여 단일 줄로 표시하고 앰버 칩도 이 줄에 부여(과거엔 기술 줄 + `LogEntry` 정보 줄이 별도 2줄로 나가 칩이 기술 줄에만 붙는 문제가 있었음, 2026-07-28 통합) |

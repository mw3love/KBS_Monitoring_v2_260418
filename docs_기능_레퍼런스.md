...
#### 탭 1: 영상 설정
- 포트 선택 (0~31) 또는 비디오 파일 선택 (MP4, AVI 등)
- 자동 녹화 설정 (활성화, 저장 폴더, 사전/사후 시간, 보관 일수, 출력 해상도/FPS)

#### 탭 2: 비디오 영역 설정
- ROI 테이블 (Label, Media Name, X, Y, Width, Height)
- 버튼: 추가 / 삭제 / 복사 (+20,+20) / 위로 / 아래로 / 초기화 / 편집(반화면 오버레이)

#### 탭 3: 오디오 영역 설정
- 비디오 영역 탭과 동일 구조 (A1, A2, ... 레이블)
...
- 오디오 레벨미터(HSV) 파라미터
- 임베디드 오디오 파라미터
- 성능 설정 (감지 주기, 해상도 스케일, 타입별 활성화)

#### 탭 5: 알림 설정
- 알림음 (활성화, 볼륨, 테스트 재생)
- 텔레그램 (활성화, Bot Token, Chat ID, 이미지 전송, 쿨다운, 타입별 플래그, 연결 테스트)

#### 탭 6: 정파 설정
- 자동 정파준비 활성화
- 그룹 1/2 각각: 그룹명, 시작/종료 시각, 익일 종료, 정파준비 시간, 정파해제준비, 요일 선택, 진입 ROI, 억제 ROI 목록, 해제 트리거

#### 탭 7: 저장/불러오기
- 현재 설정 저장 (파일 다이얼로그 → JSON)
- 설정 불러오기 (JSON → 즉시 적용)
- 설정 초기화 (확인 다이얼로그 → 기본값 적용)
...
| pre_seconds | 5 | 1~60 | 초 |
| post_seconds | 15 | 1~60 | 초 |
| max_keep_days | 7 | 1~30 | 일 |

...
> ⏳ **관찰 중**: v2.8.8 — 자동녹화 영상이 알람 발생 후 구간에서 실제 캡처 fps 그대로(throttle 누락) 기록돼 슬로우모션+오디오 어긋남("2초 뒤 소리 시작, 점점 커짐") 발생하던 버그 수정. 합성 프레임 시뮬레이션까지만 검증, 실제 카메라+ffmpeg 녹화본 재생 확인은 다음 알람 발생 시 현장에서.

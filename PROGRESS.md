...
> 마지막 업데이트: 2026-08-12 (UI 손상 6차 재발 — 원인은 Round 2 자체의 실패가 아니라 2026-08-07 재배포가 `.venv313`을 조용히 지운 것. `fix/260526_...md` §14)
...
> ⚠ **판정 재개 (2026-08-12)**: W17 Round 2(Python 3.13 다운그레이드) — §13-10의 "재발 없음 확정"(2026-08-07)은 07-25~08-07 13일 표본으로선 유효하나, **그 직후 재배포가 `.venv313`을 조용히 지워** 08-07~08-11 사이 아무도 모르게 Python 3.14로 돌다가 08-11 09:33 **6차 재발**. Round 2 결론(범인은 Python 3.14 계열) 자체는 재확증됨. `.venv313` 재생성 + 재기동(08-12 09:49, python=3.13.14 확인) 완료, 판정 시계 재시작 — 다음 위험창 ~08-16~17. 상세: `fix/260526_설정다이얼로그_TypeError_재현불가.md` §14.
> ⏳ **관찰 중**: v2.8.8 — 자동녹화 영상이 알람 발생 후 구간에서 실제 캡처 fps 그대로(throttle 누락) 기록돼 슬로우모션+오디오 어긋남("2초 뒤 소리 시작, 점점 커짐") 발생하던 버그 수정. 합성 프레임 시뮬레이션까지만 검증, 실제 카메라+ffmpeg 녹화본 재생 확인은 다음 알람 발생 시 현장에서. **07-25~08-07 무알람 기간 동안 검증 기회 없었음(미검증 유지).**
...
### P0 추가 — UI 장기가동 손상 4차 대응 (v2.8.0, 2026-07-13) ✅ Round 2 판정 완료(2026-08-07)
...
- [x] **Round 2 판정 — 통과 확정 (2026-08-07, 운영 PC 로그 전수 검토)** — `logs/2026072[5-9]*`, `2026080[1-7]*` 회수해 검토. 07-25 12:01:04(설정 확정 직후) 이후 **UI/Watchdog 재시작 0회, Detection 재spawn 0회, `ui_degraded.flag` 0건, heartbeat 무응답 0건, ERROR/CRITICAL 0건, fault*.log 전부 0바이트**, HEALTH 스냅샷 하루 144개 누락 없이 정상 — 13일 연속가동이 과거 5회 온셋(4.15~5.48일)을 전부 넘김. **Python 3.14 계열(PySide6 자체 포함)이 범인이었던 것으로 결론, Round 3(PySide6 다운그레이드) 불필요.** 상세: `fix/260526_...md` §13-10.
  - ⚠ 이 기간 **실제 알람 0건**(전부 정파 시간대 억제된 still/black) — 힙손상 부재 판정은 유효하나, 알람+녹화 경로(v2.8.8 포함)는 이 무사고 기록으로 검증되지 않음.
- [x] **UI 프로세스 완만한 메모리 증가 발견 및 수정 (2026-08-07)** — Round 2 판정 중 HEALTH 로그에서 Detection/Watchdog은 평평한데 **UI만** 13일간 RSS 198→373MB(+88%), handle 988→1092 계단식 증가 관찰(평일 낮 시간대에 점프 후 유지 — 사용자 조작과 연관 추정). 코드 확인 결과 `_open_settings()`가 열 때마다 새 `SettingsDialog`를 만들면서 **이전 인스턴스를 한 번도 해제하지 않음**(`Qt.WA_DeleteOnClose` 미설정, `closeEvent`에 `deleteLater()` 없음) — Qt 부모(MainWindow)에 계속 매달려 숨은 채 누적되는 전형적 패턴. 이전 인스턴스 `deleteLater()` 호출 + 열림/닫힘 로그(`[settings] 설정창 열림/닫힘`) 추가로 수정. — `ui/main_window.py` `_open_settings()`.
  - 검증(프록시): 독립 하네스로 열기/닫기 6회 반복 — 자식 위젯 수가 무한 증가하지 않고 800→1599(과도기)→종료 후 800(기준선 복귀)로 수렴 확인, 로그 열림/닫힘 쌍 정상 기록. **실제 운영 PC에서 수개월 단위 장기 추세로 RSS가 실제로 안정되는지는 다음 HEALTH 로그 검토가 실조건검증.**
- [x] **UI 손상 6차 발생 — 원인: 배포가 Round 2 픽스를 조용히 무효화 (2026-08-11 onset, 2026-08-12 운영 PC 직접 조사·조치)** — 2026-08-07 13:21 재배포가 **폴더째 새로 clone**한 방식이었는데 `.venv313/`이 `.gitignore` 대상이라 안 따라옴 → `실행.bat`이 경고 없이 시스템 기본 Python(3.14.3)으로 폴백 → 3.84일 뒤(08-11 09:33) 힙손상 재발. 이번엔 로그 복사 추측이 아니라 운영 PC(이 PC) 위에서 프로세스 커맨드라인·`git reflog`·기동 로그 배너를 직접 교차확인해 확정. **처음으로 onset 실제 traceback 확보**: `ui/alarm.py:187 play_signoff_sound`(정파 전환 알림음)의 `threading.Thread(...)` 생성이 트리거. `PYTHONMALLOC=debug` 전수(3.84일치)에서 가드바이트 위반 0건 — 단순 버퍼오버플로는 아님. 상세: `fix/260526_...md` §14.
  - **조치**: `.venv313` 재생성(`py -3.13 -m venv` + `requirements.txt` 재설치) → 재기동 → `python=3.13.14` 확인, `ui_degraded.flag` 해소. `실행.bat`이 `.venv313` 부재 시 콘솔+`logs/stderr_debug.txt`에 NOTICE를 남기도록 수정(더 이상 조용한 폴백 없음). `CLAUDE.md` "신규 빌드 배포 시 함정"에 `.venv313` 항목 추가.
  - ⚠ **미검증**: `.venv313` 재생성이 이번에도 위험창(~4~5일, ~08-16~17)을 무사히 넘기는지는 다음 판정까지 미확정. 다음 재배포도 "폴더째 재clone" 방식이면 같은 함정이 재발할 수 있음 — 재배포 직후 그날 UI 로그의 `EXPERIMENT: ... python=` 배너 확인을 배포 체크리스트에 반영할 것.
